...
>质量压缩的关键在于Bitmap.compress()函数，该函数不会改变图片的大小，但是可以降低图片的质量，从而降低存储大小，进而达到压缩的目的。
...
- CompressFormat format：压缩格式，它有JPEG、PNG、WEBP三种选择，JPEG是有损压缩，PNG是无损压缩，压缩后的图片大小不会变化（也就是没有压缩效果），WEBP是Google推出的
图片格式，它相比JPEG会节省30%左右的空间，处于兼容性和节省空间的综合考虑，我们一般会选择JPEG。
- int quality：0~100可选，数值越大，质量越高，图片越大。
- OutputStream stream：压缩后图片的输出流。                                            
...
可以看到随着quality的降低，图片质量发生了明显的变化，但是图片的尺寸没有发生变化。这个方法是在Java层是很简单的，我们来探究它的底层实现原理。
...
```
可以看到该函数根据编码格式选择SkImageEncoder，从而创建对应的图片编码器，最后调用encodeStream(strm, *bitmap, quality)方法来完成编码。通过名字我们就可以
看出这是Google的Skia图形库。
...
我们虽然在平时的开发中没有直接用到Skia，但它对我们太重要了，它是Android系统的重要组成部分，很多重要操作例如图片编解码，Canvas绘制在底层都是通过Skia来完成的。它同样被
广泛用于Google的其他产品中。

Skia本身提供了基本的画图和编解码功能，它同时还挂载了其他第三方编解码库，例如：libpng.so、libjpeg.so、libgif.so、所以我们上面想要编码成jpeg图片最终是由libjpeg来完成的。
...
- [mozilla/mozjpeg](https://github.com/mozilla/mozjpeg)：基于libjpeg-turbo.实现，保证不降低图片质量且兼容主流编解码器的情况下进行jpeg压缩。

## 二 尺寸压缩

>尺寸压缩本质上就是一个重新采样的过程，放大图像称为上采样，缩小图像称为下采样，Android提供了两种图片采样方法，邻近采样和双线性采样。
...
可以看到这种方式的关键在于inSampleSize的选择，它决定了压缩后图片的大小。

>inSampleSize代表了压缩后的图片一个像素点代表了原来的几个像素点，例如inSampleSize为4，则压缩后的图片的宽高是原来的1/4，像素点数是原来的1/16，inSampleSize
