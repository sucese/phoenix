...
public class CompressImageUtil {
    ...
    public CompressImageUtil(Context context, PhoenixOption config) {
        this.context = context;
        this.config = config;
    }

    public void compress(String imagePath, CompressListener listener) {
        if (config.isCompreEnablePixel()) {
            try {
    ...
    private void compressImageByQuality(final Bitmap bitmap, final String imgPath, final CompressListener listener) {
        ...
        new Thread(new Runnable() {//开启多线程进行压缩处理
            ...
            public void run() {
                ...
                while (baos.toByteArray().length > config.getCompressMaxSize()) {//循环判断如果压缩后图片是否大于指定大小,大于继续压缩
...
//				}
    ...
    private void compressImageByPixel(String imgPath, CompressListener listener) throws FileNotFoundException {
        ...
        float maxSize = config.getCompressMaxPixel();
        int be = 1;
        ...
        if (config.isCompressEnableQuality()) {
